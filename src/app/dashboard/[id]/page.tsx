...
import {
  ...
  Check, Search, Plus, ChevronDown, Hash, AtSign, Volume2, Info
} from "lucide-react";
...
const SearchableSelect = ({ options, value, onChange, placeholder, isRole = false, t }: any) => {
  ...
  const filtered = q === "" ? options : options.map((group: any) => {
    if (group.items) return { ...group, items: group.items.filter((i: any) => i.name.toLowerCase().includes(q.toLowerCase())) };
    return group.name?.toLowerCase().includes(q.toLowerCase()) ? group : null;
      ...
      <button type="button" onClick={() => { setOpen(!open); setQ(""); }}
        ...
        <span className="flex items-center gap-2 truncate">
          {selected ? (<>
            ...
            <span className={`${isRole ? 'text-gray-200' : 'text-gray-200'}`}>{selected.name}</span>
...
/* ─────────────────────────────────────────────
   COMPONENT: Modal
   ───────────────────────────────────────────── */
const Modal = ({ isOpen, onClose, title, children, onSave, saveLabel }: any) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0 bg-black/70" />
      <div className="bg-[#313338] border border-[#1e1f22] rounded-lg shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col relative animate-scale-up" onClick={e => e.stopPropagation()}>
...
const Card = ({ children, className = "" }: any) => (
  <div className={`bg-[#2b2d31] rounded-lg border border-[#1e1f22] ${className}`}>{children}</div>
);
...
const Input = ({ value, onChange, placeholder, type = "text", ...rest }: any) => (
  <input type={type} value={value || ""} onChange={e => onChange(type === "number" ? parseInt(e.target.value) || 0 : e.target.value)} placeholder={placeholder}
    className="w-full bg-[#1e1f22] border border-[#3f4147] hover:border-[#5865F2] focus:border-[#5865F2] rounded-[4px] px-3 py-[10px] text-sm text-white focus:outline-none transition-colors" {...rest} />
);
  ...
  useEffect(() => {
    fetch(`/api/bot/guilds/${id}/config`)
      ...
      .then(d => {
        ...
        ["ticket_categories"].forEach(k => { if (!d[k]) d[k] = {}; });
  ...
  return (
    <div className="max-w-[1300px] mx-auto animate-fade-in-up pb-28">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link href="/dashboard" className="text-gray-500 hover:text-gray-300 flex items-center gap-1.5 text-sm transition mb-1">
        ...
        <div className="flex-1 min-w-0">
          ...
          {tab === "general" && (
            <div className="animate-fade-in space-y-6">
              <div><h2 className="text-lg font-semibold text-white">{t("tab_general")}</h2></div>
              <Card className="p-5 max-w-xl">
                <Field label={t("general_lang")} description={t("general_lang_desc")}>
                  <select value={config.language || "en"} onChange={e => set("language", e.target.value)}
                    ...
                    {[["en","🇬🇧 English"],["fr","🇫🇷 Français"],["es","🇪🇸 Español"],["de","🇩🇪 Deutsch"],["it","🇮🇹 Italiano"],["pt","🇵🇹 Português"],["ru","🇷🇺 Русский"],["ja","🇯🇵 日本語"]].map(([v,l]) => (
          ...
          {tab === "welcome" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("welcome_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("welcome_desc")}</p>
              </div>

              <div className="flex flex-col xl:flex-row gap-6">
                <Card className="p-5 flex-1 space-y-5">
                  <Field label={t("welcome_channel")}>
                    <SearchableSelect value={config.welcome?.channel_id} onChange={(v: any) => setN("welcome", "channel_id", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                  </Field>
                  <Field label={t("welcome_autorole")}>
                    <SearchableSelect value={config.welcome?.auto_roles?.[0]} onChange={(v: any) => setN("welcome", "auto_roles", v ? [v] : [])} options={config.discord_roles} isRole t={t} placeholder={t("select_role")} />
                  </Field>
                  <Field label={t("welcome_msg")} description={t("welcome_vars")}>
                    <Textarea value={config.welcome?.message} onChange={(v: any) => setN("welcome", "message", v)} rows={4} />
                  </Field>
                  <Field label={t("welcome_bg")}>
                    <Input value={config.welcome?.background_url} onChange={(v: any) => setN("welcome", "background_url", v)} placeholder="https://i.imgur.com/..." />
                  </Field>
                </Card>
          ...
          {tab === "automod" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("automod_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("automod_desc")}</p>
              </div>
              ...
              <Card className="p-5 max-w-2xl space-y-4">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Advanced Settings</p>
                <div className="grid grid-cols-3 gap-4">
                  <Field label={t("automod_caps_threshold")}>
                    <Input type="number" value={config.automod?.caps_threshold ?? 70} onChange={(v: any) => setN("automod", "caps_threshold", v)} />
                  </Field>
                  <Field label={t("automod_caps_minlen")}>
                    <Input type="number" value={config.automod?.caps_min_length ?? 8} onChange={(v: any) => setN("automod", "caps_min_length", v)} />
                  </Field>
                  <Field label={t("automod_max_mentions")}>
                    <Input type="number" value={config.automod?.max_mentions ?? 5} onChange={(v: any) => setN("automod", "max_mentions", v)} />
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <Field label={t("automod_allowed_links")}>
                    <SearchableSelect value={config.automod?.allowed_links_roles?.[0]} onChange={(v: any) => setN("automod", "allowed_links_roles", v ? [v] : [])} options={config.discord_roles} isRole t={t} placeholder={t("select_role")} />
                  </Field>
                  <Field label={t("automod_allowed_invites")}>
                    <SearchableSelect value={config.automod?.allowed_invites_roles?.[0]} onChange={(v: any) => setN("automod", "allowed_invites_roles", v ? [v] : [])} options={config.discord_roles} isRole t={t} placeholder={t("select_role")} />
                  </Field>
                </div>
              </Card>
            </div>
          ...
          {tab === "tickets" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("tickets_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("tickets_desc")}</p>
              </div>

              <div className="grid md:grid-cols-2 gap-4 max-w-2xl">
                <Card className="p-5">
                  <Field label={t("tickets_panel")} description={t("tickets_panel_desc")}>
                    <SearchableSelect value={config.tickets_panel_channel} onChange={(v: any) => set("tickets_panel_channel", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                  </Field>
                </Card>
                <Card className="p-5">
                  <Field label={t("tickets_logs")} description={t("tickets_logs_desc")}>
                    <SearchableSelect value={config.tickets_logs_channel} onChange={(v: any) => set("tickets_logs_channel", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                  </Field>
                </Card>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setEditTicket({ id: "cat_" + Date.now(), name: "", description: "", emoji: "🎫", staff_roles: [], questions: [], open_message: "Welcome {user}, a staff member will assist you shortly." })}
                  className="bg-[#5865F2] hover:bg-[#4752C4] text-white px-3 py-1.5 rounded-[4px] text-sm font-medium transition flex items-center gap-1.5">
                  <Plus size={14} /> {t("tickets_add_cat")}
                </button>
              </div>
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
                {Object.entries(config.ticket_categories || {}).map(([cid, cat]: [string, any]) => (
                  <Card key={cid} className="p-4 hover:border-[#5865F2] transition-colors cursor-pointer group relative" onClick={() => setEditTicket({ id: cid, ...cat })}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-white truncate">{cat.name || "Untitled"}</p>
                ...
                {Object.keys(config.ticket_categories || {}).length === 0 && (
                  <div className="col-span-full py-12 text-center text-gray-600 border-2 border-dashed border-[#2b2d31] rounded-lg text-sm">
                    No ticket categories yet. Create one to get started.
                  </div>
                )}
              </div>
            </div>
          ...
          {tab === "supporter" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("supporter_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("supporter_desc")}</p>
              </div>
              <Card className="p-5 max-w-xl space-y-5">
                <Field label={t("supporter_role")} description={t("supporter_role_desc")}>
                  <SearchableSelect value={config.supporter_role_id} onChange={(v: any) => set("supporter_role_id", v)} options={config.discord_roles} isRole t={t} placeholder={t("select_role")} />
                </Field>
                <Field label={t("supporter_keyword")} description={t("supporter_keyword_desc")}>
                  <Input value={config.supporter_keyword} onChange={(v: any) => set("supporter_keyword", v)} placeholder={t("supporter_keyword_ph")} />
                </Field>
              </Card>
            </div>
          )}
          ...
          {tab === "shop" && (
            <div className="animate-fade-in space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-white">{t("shop_title")}</h2>
                  <p className="text-sm text-gray-500 mt-0.5">{t("shop_desc")}</p>
                </div>
                <button onClick={() => setEditShop({ id: Math.random().toString(36).substring(7), name: "", price: 100, stock: -1, description: "", roles: [] })}
                  className="bg-[#5865F2] hover:bg-[#4752C4] text-white px-3 py-1.5 rounded-[4px] text-sm font-medium transition flex items-center gap-1.5">
                  <Plus size={14} /> {t("shop_add")}
                </button>
              </div>
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
                {(config.shop || []).map((item: any, idx: number) => (
                  <Card key={item.id || idx} className="overflow-hidden hover:border-[#5865F2] transition-colors cursor-pointer" onClick={() => setEditShop({ ...item, _index: idx })}>
                    <div className="px-4 py-3 bg-[#1e1f22] flex items-center justify-between border-b border-[#1e1f22]">
                      <p className="font-medium text-white truncate text-sm">{item.name || "Untitled"}</p>
                      <span className="text-[#5865F2] text-xs font-bold whitespace-nowrap">{item.price} 🪙</span>
                    </div>
                    <div className="px-4 py-3">
                      <p className="text-xs text-gray-500 line-clamp-2 mb-2">{item.description || "—"}</p>
                      <div className="flex items-center gap-2 text-[11px]">
                        <span className="text-gray-500">Stock: <span className="text-gray-300">{item.stock === -1 ? "∞" : item.stock}</span></span>
                        {item.roles?.length > 0 && <span className="bg-green-500/10 text-green-400 px-1.5 py-0.5 rounded">+{item.roles.length} role</span>}
                      </div>
                    </div>
                  </Card>
                ))}
          ...
          {tab === "voice" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("voice_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("voice_desc")}</p>
              </div>
              ...
              <Card className="p-5 max-w-xl">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <Field label={t("voice_afk_timeout")}>
                    <Input type="number" value={config.voice_afk?.timeout_minutes ?? 15} onChange={(v: any) => setN("voice_afk", "timeout_minutes", v)} />
                  </Field>
                  <Field label={t("voice_afk_channel")}>
                    <SearchableSelect value={config.voice_afk?.afk_channel_id} onChange={(v: any) => setN("voice_afk", "afk_channel_id", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                  </Field>
                </div>
              </Card>
              ...
              <Card className="p-5 max-w-xl">
                ...
                <Field label={t("voice_hub_channel")}>
                  <SearchableSelect value={config.voice_hub_channel} onChange={(v: any) => set("voice_hub_channel", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                </Field>
              </Card>
            </div>
          )}
          ...
          {tab === "logs" && (
            <div className="animate-fade-in space-y-6">
              <div>
                <h2 className="text-lg font-semibold text-white">{t("logs_title")}</h2>
                <p className="text-sm text-gray-500 mt-0.5">{t("logs_desc")}</p>
              </div>

              <div className="bg-[#2d2000] border border-[#5a4000] rounded-lg px-4 py-3 max-w-xl flex items-start gap-3">
                <Info size={18} className="text-yellow-500 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-yellow-200/80">{t("logs_warn")}</p>
              </div>

              <Card className="p-5 max-w-xl">
                <Field label={t("logs_channel")} description={t("logs_channel_desc")}>
                  <SearchableSelect value={config.logs_channel} onChange={(v: any) => set("logs_channel", v)} options={groupedChannels} t={t} placeholder={t("select_channel")} />
                </Field>
              </Card>
            </div>
          )}
        ...
        <div className="bg-[#111214] text-white px-5 py-3 rounded-lg shadow-[0_-4px_32px_rgba(0,0,0,0.6)] flex items-center justify-between w-full max-w-xl mx-4 mb-5 border border-[#1e1f22] pointer-events-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={save} disabled={saving}
              className="bg-[#23A559] hover:bg-[#1f934f] text-white px-4 py-1.5 rounded-[4px] text-sm font-medium transition disabled:opacity-50 flex items-center gap-1.5">
      ...
      <Modal isOpen={!!editTicket} onClose={() => setEditTicket(null)} title={editTicket?.name ? `${t("edit")} — ${editTicket.name}` : t("tickets_add_cat")}
        onSave={() => { const n = { ...(config.ticket_categories || {}) }; n[editTicket.id] = { name: editTicket.name, description: editTicket.description, emoji: editTicket.emoji, staff_roles: editTicket.staff_roles, questions: editTicket.questions, open_message: editTicket.open_message }; set("ticket_categories", n); setEditTicket(null); }}>
        {editTicket && (<>
          <div className="flex gap-3">
          ...
          <Field label={t("tickets_cat_staff")}><SearchableSelect value={editTicket.staff_roles?.[0]} onChange={(v: any) => setEditTicket({ ...editTicket, staff_roles: v ? [v] : [] })} options={config.discord_roles} isRole t={t} placeholder={t("select_role")} /></Field>
          ...
          <div className="border-t border-[#1e1f22] pt-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t("tickets_cat_questions")}</p>
              <button onClick={() => setEditTicket({ ...editTicket, questions: [...(editTicket.questions || []), { label: "", required: true }] })} className="text-[#5865F2] text-xs font-medium hover:underline">+ {t("tickets_add_question")}</button>
            </div>
            ...
            {(editTicket.questions || []).map((q: any, i: number) => (
              <div key={i} className="flex gap-2 mb-2 items-center">
                <Input value={q.label} onChange={(v: any) => { const nq = [...editTicket.questions]; nq[i] = { ...nq[i], label: v }; setEditTicket({ ...editTicket, questions: nq }); }} placeholder={t("tickets_question_label")} />
                <button onClick={() => { const nq = [...editTicket.questions]; nq.splice(i, 1); setEditTicket({ ...editTicket, questions: nq }); }} className="text-gray-600 hover:text-red-400 p-1.5 transition"><Trash2 size={14} /></button>
              </div>
            ))}
          </div>
        </>)}
      ...
      <Modal isOpen={!!editShop} onClose={() => setEditShop(null)} title={editShop?._index !== undefined ? t("modify") : t("shop_add")}
        ...
        {editShop && (<>
          ...
          {editShop._index !== undefined && (
            <div className="pt-3 border-t border-[#1e1f22]">
              <button onClick={() => { const ns = [...config.shop]; ns.splice(editShop._index, 1); set("shop", ns); setEditShop(null); }} className="text-red-400 hover:underline text-xs">{t("shop_delete")}</button>
            </div>
          )}
        </>)}
      </Modal>
